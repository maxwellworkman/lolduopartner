...
function WinRates(props) {
  ...
  const animatedStyles = useSpring({
    ...
    from: { width: "0%" },
    config: { tension: 300, friction: 50, delay: 2500 },
  });
    ...
    <animated.div className="leftStats" style={fadeInAnimation}>
      {Object.keys(sortedWinRates).length > 0 ? (
        <div className="win-rates-container">
          {Object.entries(sortedWinRates).map(
            ([pair, { champion1, champion2, winRate }]) => (
              <div className="pairRow" key={pair}>
                <div className="portraitBox">
                  <div className="wrPortraitContainer">
                    <ChampionPortrait championName={champion1} />
                  </div>
                  <div className="wrPortraitContainer">
                    <ChampionPortrait championName={champion2} />
                  </div>
                </div>
                <div className="percentageBox">
                  <div
                    className="percentageBar"
                    style={{ width: `${(winRate * 100).toFixed(2)}%` }}
                  >
                    <animated.div
                      className="percentageBarAnimated"
                ...
                {/* add games Played and maybe kda */}
              </div>
            )
          )}
        </div>
